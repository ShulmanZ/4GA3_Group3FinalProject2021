...
<<<<<<< HEAD
Mutate to make variables rates:
...
```{r}
=======
```{r Mutate-Field-For-SMA, include=FALSE}
>>>>>>> 9e559cd2c6528b3f57a56051c644d97fb3c259ca
Neighbourbounds <- Neighbourbounds %>%
  mutate(SMA_MH_PC_Tot = MH_PC_Tot,
         Prop_Comp_Avg_Income = Avg_Ind_In / 46530 )


```

#Spatial weights Matrix for data
```{r Create-Spatial-Continguities, include=FALSE}
...
```{r Create-Buffers, include=FALSE}
#5 minute walk 
...
```{r}
=======
```{r Population-Density-Map, include=FALSE}
>>>>>>> 9e559cd2c6528b3f57a56051c644d97fb3c259ca
...
```{r}
=======
```{r Mental-Health-Cases-Maps, include=FALSE}
>>>>>>> 9e559cd2c6528b3f57a56051c644d97fb3c259ca
...
```{r}
=======
```{r Unemployment-Rate-Maps, include=FALSE}
>>>>>>> 9e559cd2c6528b3f57a56051c644d97fb3c259ca
#Unemployment Rates 
...
```{r Living-Situation-Maps, include=FALSE}
#Living Situation 
...
```{r Mutation-To-Create-KM^2-Data, include=FALSE}
Neighbourbounds <- Neighbourbounds %>%
...
```{r Avaliable-Green-Space-Within-Buffer-Distance, include=FALSE}
options(scipen=10000)
...
```{r Age-Proportion-Maps, include=FALSE}
...
Moran Plot:
```{ moran-scatterplot, echo=FALSE, fig.cap={"\\label{fig:moran-scatterplot}Moran's scatterplot of empirical variable (observed)"}}

MHTot_MP <- moran.plot(Neighbourbounds$MH_PC_Tot, Neighbourbounds.w)
```

this function is used to create local Moran maps:
```{r Local-Moran-Map, include=FALSE}
...
```{r Addition-of-SMA-To-Dataframe, include=FALSE}
...
modelBest <- lm(formula = MH_PC_Tot ~Area_5_Transform * Unemploy_r * Prop_Alone,  data = Neighbourbounds)
summary(modelBest)
...
model4 <- lm(formula = MH_PC_Tot ~Area_5_Transform * Unemploy_r * Prop_Alone * PropAge65Plus * Med_Ind_In * Immigrants, data = Neighbourbounds)
summary(model4)
