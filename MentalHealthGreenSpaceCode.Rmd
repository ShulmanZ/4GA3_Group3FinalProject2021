...
output:
  ...
  pdf_document:
    template: project-template-default.tex
abstract: This paper reports our analysis of the correlation between green space and
  mental health within Toronto, ON, and its relationship with other socioeconomic
  factors. Data was collected from the Open Ontario, statistics Canada, and Toronto
  Community Health.
...
#Fixing the names up for some of the fieldsas required
...
#Creating new rate variables from the absolute variables obtained
...
#Adding a field to run spatial moving average on
Neighbourbounds <- Neighbourbounds %>%
...
# Creatin Spatial weights Matrix for data
...
```{r plot-density, echo=FALSE, fig.cap={"\\label{fig:Pop-Dens}Population Density based on Total Population of 2615060 in Toronto, ON"}}
...
ggplot(Neighbourbounds) + 
  ...
  labs(fill = "Population Density (per km^2)")
...
```{r plot-cases, echo=FALSE, fig.cap={"\\label{fig:Ment-Cases-Cap}Mental Health Cases per Capita in Toronto, ON"}}
...
```{r plot-male-cases, echo=FALSE, fig.cap={"\\label{fig:Ment-MCases-Cap}Male Mental Health Cases per Capita in Toronto, ON"}}
options(scipen=10000)
ggplot(Neighbourbounds) + 
...
```{r plot-female-cases, echo=FALSE, fig.cap={"\\label{fig:Ment-FCases-Cap}Female Mental Health Cases per Capita in Toronto, ON"}}
options(scipen=10000)
ggplot(Neighbourbounds) + 
...
```{r plot-unemployment-rates, echo=FALSE, fig.cap={"\\label{fig:Unemploy-Rate}Unemployment Rates in Toronto, ON Neighbourhoods"}}
#Unemployment Rates 
...
```{r plot-income, echo=FALSE, fig.cap={"\\label{fig:plot-income}Median Individual Income in Toronto, ON Neighbourhoods"}}
...
```{r plot-Living-Alone, echo=FALSE, fig.cap={"\\label{fig:plot-alone}Proportion of Population Living Alone in Toronto, ON Neighbourhoods"}}
#Living Situation 
options(scipen=10000)
ggplot(Neighbourbounds) + 
  geom_sf(aes(fill = cut_number(((Alone) / Census_Pop)*100,8)),
          ...
          size = 0.1) +
  scale_fill_brewer(palette = "YlOrRd") +
  labs(fill = "Residents Living Alone (%)")
...
```{r plot-immigrants, echo=FALSE, fig.cap={"\\label{fig:plot-immi}Percent of Immigrated Population in Toronto, ON Neighbourhoods"}}
options(scipen=10000)
...
```{r plot-Prop-Old, echo=FALSE, fig.cap={"\\label{fig:plot-old}Proportion of Population over 65 years old in Toronto, ON"} }
#Proportion Above 65
...
ggplot() + 
  ...
  scale_fill_distiller(palette = "YlOrRd", direction = 1) +  
  labs(fill = "Proportion of Population Aged 65+") + 
  theme(axis.text.x = element_blank(), 
        axis.text.y = element_blank())
```
...
```{r plot-green-space5, echo=FALSE, fig.cap={"\\label{fig:plot-g5}Amount of greenspace within 5 minute walking buffer based on average walking speed in Toronto, ON neighbourhoods"}}
options(scipen=10000)
...
ggplot(Neighbourbounds) + 
  geom_sf(aes(fill = cut_number(Area_5min_km, 8)),
          ...
          size = 0.1) +
  scale_fill_brewer(palette = "YlOrRd") +
  labs(fill = "Greenspace within 5 minutes (km^2)")
```
...
```{r plot-green-space10, echo=FALSE, fig.cap={"\\label{fig:plot-g10}Amount of greenspace within 10 minute walking buffer based on average walking speed in Toronto, ON neighbourhoods"}}
options(scipen=10000)
ggplot(Neighbourbounds) + 
  geom_sf(aes(fill = cut_number(Area_10min_km,8)),
          ...
          size = 0.1) +
  scale_fill_brewer(palette = "YlOrRd") +
  labs(fill = "Greenspace within 10 minutes (km^2)")
```
...
```{r plot-green-space15, echo=FALSE, fig.cap={"\\label{fig:plot-g15}Amount of greenspace within 15 minute walking buffer based on average walking speed in Toronto, ON neighbourhoods"}}
options(scipen=10000)
ggplot(Neighbourbounds) + 
  geom_sf(aes(fill = cut_number(Area_15min_km,8)),
          ...
          size = 0.1) +
  scale_fill_brewer(palette = "YlOrRd") +
  labs(fill = "Greenspace within 15 minutes (km^2)")
```
...
```{r plot-green-space30, echo=FALSE, fig.cap={"\\label{fig:plot-g30}Amount of greenspace within 30 minute walking buffer based on average walking speed in Toronto, ON neighbourhoods"}}
options(scipen=10000)
ggplot(Neighbourbounds) + 
  geom_sf(aes(fill = cut_number(Area_30min_km,8)),
          ...
          size = 0.1) +
  scale_fill_brewer(palette = "YlOrRd") +
  labs(fill = "Greenspace within 30 minutes (km^2)")
...
ggplot(Neighbourbounds) + 
  geom_sf(aes(fill = cut_number(Area_60min_km, 8)),
          ...
          size = 0.1) +
  scale_fill_brewer(palette = "YlOrRd") +
  labs(fill = "Greenspace within 60 minutes (km^2)")
...
Neighbourhood_centroids <- st_centroid(Neighbourbounds_Centroids) %>% 
  st_coordinates()
```
```{r Neighbourhood-SWM5,echo=FALSE, fig.cap={"\\label{fig:neighbour-SWM5}Spatial weights matrices of neighbouring neighbourhoods within a 5 minute walking distance; the lack of links indicates no neighbourhood centroids are within 5 minutes of each other"}}
...
Below is the results of the linear model the relationship between all socioeconomic factors (including green space avaliability within 5 minute walking distance of neighbourhoods) and mental health related hospital admissions:

```{r Linear-Regression-All,echo=FALSE, fig.cap={"\\label{fig:LR-All}The relationship between all socioeconomic factors (including green space avaliability within 5 minute walking distance of neighbourhoods) and mental health related hosptial admissions"}}
#Model with everything
testmodel <- lm(formula = MH_PC_Tot ~ Area_5_Transform + Unemploy_r + Prop_Alone + PropAge65Plus + Immigrants, data = Neighbourbounds)


stargazer(testmodel, type = "text")
```


Below is the results of the relationship between all socioeconomic factors (not including green space avaliability within 5 minute walking distance of neighbourhoods) and mental health related hospital admissions:

```{r Linear-Regression-NoGreen,echo=FALSE, fig.cap={"\\label{fig:LR-NoGreen}The relationship between all socioeconomic factors (not including green space avaliability within 5 minute walking distance of neighbourhoods) and mental health related hosptial admissions"}}
#Model with everything minus green space
testmodel <- lm(formula = MH_PC_Tot ~  Unemploy_r + Prop_Alone + PropAge65Plus + Immigrants, data = Neighbourbounds)
...
- The availability of greenspace was not found to be influential on a neighbourhood's mental health when other socioeconomic factors including  high immigrant populations, high unemployment rates, low annual average incomes, high proportion of people living alone or senior citizens (over 65 years old) were input into the regression model. This may be due to the fact that a neighbourhoods mental health was measured by a number of hospital admission related to mental health problems. This implies more serious mental health issues and may not be representative of the neighbourhood's actual mental health in relation to green space availability. Future research could focus on collecting a bigger range of mental welling through activities such as surveys, and then comparing this to green space availability.  
