...
Adding the area of parks into the neighbourhood:
```{r}
#Using intersect tool, identify the park space that intersects with/ is within each buffer:

intersect_15min = st_intersection(green_buff_15min, Parks)
int15 <- subset(intersect_15min, select = c(FIELD_6, NAME))
```
```{r}
int15 <- subset(intersect_15min, select = c(FIELD_6, NAME))
```
```{r}
myData <- aggregate(int15, int15["NAME"], FUN = count)
```
